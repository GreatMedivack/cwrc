...
def get_item_title(id)
	@db.execute("select id, title, valuable from item_types where id=?", id).flatten)
		  ...
		  when Telegram::Bot::Types::Message
		    ...
		    if message.text == 'Спрятать ресурсы'
			  #  	res_hide_btn = [
	   		  ...
	   		  #     ]
			  #   markup = Telegram::Bot::Types::InlineKeyboardMarkup.new(inline_keyboard: res_hide_btn)
		    ...
		    if message.text =~ /Твой склад с материалами/
				...
				message.text.each_line do |line|
					...
					cw_id = line.slice(/^\/add_\d+ /).slice(/\d+/)
					if (100..180).include? cw_id 
						item = get_item_type(cw_id)
						id = item[0]
						title = item[1]
						valuable = item[2]
					else	
